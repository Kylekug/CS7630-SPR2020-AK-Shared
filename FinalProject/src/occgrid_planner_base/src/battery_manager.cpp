...
class BatteryManager {
    protected:
        ...
        void battery_callback(const std_msgs::Float32 & msg) {
			...
			ROS_INFO("Battery Level is %f percent", battery_charge);
			}
        
        void replanner_callback(const ros::TimerEvent& event){
		    geometry_msgs::PoseStamped pose;			
			tf::StampedTransform transform;
			geometry_msgs::PoseStamped odom_or;
			
			odom_or.pose.position.x = 0.0 ;
			odom_or.pose.position.y = 0.0 ;
			odom_or.pose.position.z = 0.0 ;
			tf::Quaternion orientation_;
	        orientation_.setEuler( 0, 0, 0 );
	        tf::quaternionTFToMsg(orientation_, odom_or.pose.orientation);
			
			
			odom_or.header.frame_id = home_frame_ ;
			odom_or.header.stamp = ros::Time::now();
            if (!ready) {
			...
			if (debug) {
				...
				listener_.waitForTransform(frame_id_,odom_or.header.frame_id,odom_or.header.stamp,ros::Duration(1.0));
				listener_.transformPose(frame_id_,odom_or, pose);
            ...
            int l = lpath.size();
            // battery check based on path generated length
            if (battery_charge < l*0.2) {
	            nav_msgs::Path path;
    ...
    public:
        BatteryManager() : nh_("~") {
            ...
            nh_.param("home_frame",home_frame_,std::string("/map"));
