...
class OccupancyGridPlanner {
    protected:
        ...
        cv::Point3i target; //----
        bool plan = false;
        geometry_msgs::PoseStampedConstPtr stp_pt;
        ros::Timer timer;
        ...
        void og_callback(const nav_msgs::OccupancyGridConstPtr & msg) {
            ...
            //ROS_INFO("og_center: %.2f %.2f %.2f",-info_.origin.position.x/info_.resolution, -info_.origin.position.y/info_.resolution,0);
			...
			//---- Erosion of obstacles           
			double erosion_size =radius/info_.resolution;
...
// ------------       
        ...
        void replanner_callback(const ros::TimerEvent& event){
			if (plan==false) {
	            ...
	            if (debug) {
	                ...
	                listener_.waitForTransform(frame_id_,stp_pt->header.frame_id,stp_pt->header.stamp,ros::Duration(1.0));
	                listener_.transformPose(frame_id_,*stp_pt, pose);
	            ...
	            target = cv::Point3i(pose.pose.position.x / info_.resolution, pose.pose.position.y / info_.resolution, (unsigned int)(round(t_yaw / (M_PI/4))) % 8)
						+ og_center_;
	            ...
	            //checking if i'm already in position
    ...
    public:
        OccupancyGridPlanner() : nh_("~") {
            ...
            nh_.param("radius",radius,5.0);
            nh_.param("replan_period",replan_period,10.0);
            switch (nbour) {
            ...
            timer = nh_.createTimer(ros::Duration(replan_period), &OccupancyGridPlanner::replanner_callback,this);
        }
};
